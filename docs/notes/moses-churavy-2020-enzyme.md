...
targets C, C++, Fortran, Julia, Rust, Swift, and other languages with an LLVM
backend.
...
the facts for loads, stores, and `memcpy`. The paper's example shows why an
...
loads, stores, calls, and pointers.

Some reverse rules need forward values. Enzyme caches values that cannot be
recomputed and otherwise tries to recompute them. It uses a cost model,
differential-use analysis, alias information, and cache reuse to reduce the
tape. When legal, it combines the forward and reverse work in one function.
...
The low-level strategy has limits. Enzyme needs IR for every function it must
differentiate, statically analyzable memory types, suitable TBAA information,
and support for the relevant instruction set. The paper identifies missing
...
Enzyme is the direct performance reference for fortad, and its strongest lesson
is the optimization boundary. Fortad keeps more Fortran type and control-flow
information than LLVM IR, yet it should still run semantics-preserving
structural optimization before derivative emission when that improves the
derivative. The `norm` example is a warning that optimizing generated code alone
can miss the decisive transformation.

Fortad also needs explicit type and activity facts. Source-level types make
many memory cases easier than they are in LLVM, but aliasing, pointers, calls,
and hidden state still need a conservative analysis. A cache policy should be
visible in generated-code evidence, with recomputation, storage, and peak memory
reported alongside runtime. Enzyme's custom-rule interface is a useful model
for foreign or opaque procedures, while its LLVM assumptions mark the boundary
