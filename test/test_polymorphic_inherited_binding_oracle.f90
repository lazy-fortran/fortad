program test_polymorphic_inherited_binding_oracle
    !! Independent oracle for one fixed concrete SELECT TYPE path whose leaf
    !! inherits the deferred binding implementation from an abstract parent.
    use fortad, only: fad_jvp, fad_vjp, fad_result_t
    implicit none

    character(len=1), parameter :: nl = achar(10)
    character(len=*), parameter :: source = &
        "module polymorphic_inherited_binding_case"//nl// &
        "    implicit none"//nl// &
        ...
        "        end select"//nl// &
        "    end function top"//nl// &
        "end module polymorphic_inherited_binding_case"//nl
    ...
    dir = "build/oracle/polymorphic_inherited_binding"
    ...
    call write_file(dir//"/derivatives.f90", &
        "module polymorphic_inherited_binding_derivatives"//nl// &
        "    use polymorphic_inherited_binding_case, only: base_t, middle_t, leaf_t"//nl// &
        "contains"//nl//jvp%code//vjp%code// &
        "end module polymorphic_inherited_binding_derivatives"//nl)
    driver = &
        "program driver"//nl// &
        "    use polymorphic_inherited_binding_case, only: base_t, leaf_t, top"//nl// &
        "    use polymorphic_inherited_binding_derivatives, only: top_jvp, top_vjp"//nl// &
        "    implicit none"//nl// &
        ...
        "    print *, 'polymorphic inherited binding oracle pass'"//nl// &
    ...
    if (stat /= 0) then
        ...
        error stop "generated inherited binding derivative did not compile"
    end if
    call execute_command_line("./"//dir//"/run > "//dir//"/out.txt 2>&1", &
        exitstat=stat)
    if (stat /= 0) then
        call show_file(dir//"/out.txt")
        error stop "inherited binding behavioral oracle failed"
    end if

    call expect_refusal(multi_dispatch_source(), "multiple dispatch", &
    ...
    print *, "test_polymorphic_inherited_binding_oracle: all cases passed"
...
contains
    ...
    function multi_dispatch_source() result(text)
        character(len=:), allocatable :: text
        text = replace_text(source, "    type, extends(middle_t) :: leaf_t"//nl, &
            "    type, extends(middle_t) :: other_t"//nl// &
            "    end type other_t"//nl// &
            "    type, extends(middle_t) :: leaf_t"//nl)
        text = replace_text(text, "        type is (leaf_t)"//nl// &
...
end program test_polymorphic_inherited_binding_oracle
