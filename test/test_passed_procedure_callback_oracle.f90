program test_passed_procedure_callback_oracle
    ...
    !!
    !! The positive case assigns one local procedure pointer once, passes it
    !! to a same-file procedure dummy, and checks hand, finite-difference, and
    !! adjoint results after both generated modes are compiled by gfortran.
    ...
    vjp = fad_vjp(source, ["x"], dependent="y", from="kernel", &
        name="kernel_vjp")
    if (.not. jvp%ok) error stop "passed-procedure JVP was refused"
    if (.not. vjp%ok) error stop "passed-procedure VJP was refused"
...
contains
    ...
    function positive_driver() result(text)
        ...
        text = "program driver"//nl// &
            ...
            "    if (abs(y-4.0625d0) > 1.0d-12) error stop 1"//nl// &
    ...
    function refusal_source(case_name) result(text)
        ...
        if (case_name /= "dynamic" .and. case_name /= "alias") then
            text = text//"        y = apply(callback, x)"//nl
        end if
