...
that contract for bounded active concrete `REAL` allocatable components and
for a fixed one-dimensional polymorphic allocatable owner array: literal
extents and one literal `SELECT TYPE` element differentiate in JVP and VJP,
with paired dynamic-type shadows, guarded owner cotangents,
finite-difference checks, and the adjoint identity. Scalar whole-component
assignment with automatic reallocation remains supported for the bounded
concrete case. Dynamic indices, sections, rank changes, factories, aliases,
pointers, ownership changes, and unresolved dispatch remain named refusals.
...
implementations, and concrete-source holder slices remain supported where
their contracts are explicit. The public facade also exposes declaration,
storage, array-bound, `SELECT TYPE`, and `SELECT RANK` facts for low-level
...
The current feature pins are FortAD `8c764ff`, FortFront `a0e109d5`, and
fortad-bench `33dff02`. The next49 evidence shard intentionally retains its
historical probe pins (`2636206` and `6c27ca86`), next50 retains
(`65280f5` and `c48c3e8b`), and next51 retains (`be82f5d` and `72bc7578`),
so those results remain reproducible; current main heads are recorded here
separately. FortAD has bounded `SPREAD` JVP and direct array-valued
reverse-transpose support, and consumes FortFront's bounded procedure-pointer
`ASSOCIATED` state facts for a fixed same-scope callback guard. It now also
consumes exact formal/actual interface facts for a fixed same-file scalar
`REAL(8)` callback or procedure pointer, preserving elemental JVP/VJP call
shape. The independent oracles check compiled JVP/VJP values, finite
...
FortFront `a0e109d5` exposes assumed-rank selector bounds identity and reports
...
FortFront `a0e109d5` also exposes formal procedure-interface signatures and
...
scalar `REAL(8)` function actual, or one same-file procedure pointer with one
unconditional assignment, passed to a scalar procedure dummy. JVP and VJP
inline the proven concrete target. Incompatible, unresolved, generic,
ambiguous, aliased, ownership-changing, and active-global callback paths remain
...
The bench currently records 1,085 of 2,014 total candidates (53.9%) and 577
of 1,432 pure-Fortran Tapenade candidates (40.3%). The remaining queue is 929
rows: 855 pure-Fortran and 74 mixed-language cases. Its latest shard is
`next51`, which records 48 modern-feature procedure roots with four generated
products, 44 explicit phase-specific refusals, and independent source-map or
refusal oracles. The pure-Fortran breakdown is 64 runnable, 134 measured
expected refusals, 34 invalid-upstream closures, 307 additional FortAD feature
boundaries, and 38 other bounded evidence closures.
...
rows. The `next11` shard closes `set03/cm31`, `set03/cm32`, `set05/v194`, and
`set06/v351` with fresh source probes, independent primal oracles, and named
FortAD boundaries.
The `next12` shard closes `set05/v058`, `set05/v176`, `set03/cmv04`, and
`set05/v175` with the same evidence contract.
...
`set10/lh232` with one invalid-generated-interface closure, one derived-type
refusal, and two pointer-alias refusals.
...
The `next18` shard adds `set01/lh099`, `set01/lh101`, `set01/lh106`, and
`set01/lh108` with explicit DO-WHILE, invalid-upstream, dependent-inference,
and COMMON/undefined-storage boundaries.
The `next19` shard adds `set01/lh110` through `set01/lh113` with legacy
inference, array-liveness, missing-reference/generated-interface, and invalid
DO-variable boundaries.
The `next20` shard adds `set01/lh114`, `set01/lh115`, `set01/lh117`, and
...
The `next22` shard adds `set01/lh123` through `set01/lh126` with four exact
legacy/control-flow and storage boundaries, each backed by pinned probes and
an independent refusal or behavioral oracle.
...
pointer-alias lifetime, missing dependents, and an invalid generated reverse
interface as separate boundaries.
The shard has independent primal, hand-derivative, finite-difference, or
refusal oracles and makes no derivative-support claim for these rows.
The preceding `next5` shard closes `set11/v540a`, `set11/v541a`, `set06/v339`,
and `set05/v197` with the same evidence contract. Earlier shards closed
`set06/v342`, `set12/mvo33`, `set11/vpf09`, and `set06/v335` in `next4`.
`set04/lh109`, `set12/mvo32`, `set12/mvo31`, and `set04/lh121` in `next3`.
and `set11/vpf19`, `set07/v479`, `set04/lh111`, and `set07/v520` in `next2`.
...
- FortFront `edcb532b`, including abstract/deferred hierarchy, concrete-only
  ...
  derived-type sources. Its abstract/polymorphic array storage oracle passes;
  ...
  storage sharing.
  Ownership-event queries now also expose source order, allocation state,
  `MOVE_ALLOC` transfer effects, and implicit reallocation flags.
  Bounded array ownership queries now retain storage identity and dynamic type
  across direct `MOVE_ALLOC` and reallocation sequences, with explicit
  global-state, alias, and unsupported-control-flow refusals.
  Its owned-array `CLASS IS` query now preserves bounded concrete implementation
  identity through local polymorphic arrays, while globals, aliases, pointers,
  and control flow remain explicit refusals.
  ...
  Its binding and call queries now expose proven inheritance depth for
  multi-level abstract/deferred implementations. The independent API oracle
  covers depths zero, one, and two and keeps generic and unresolved targets
  explicit boundaries.
  Its defined-operator query now exposes exact operand categories, kinds,
  ranks, selected specific candidates, and explicit ambiguity, conversion,
  unknown-type, pointer/TARGET, and global-state refusals for downstream
  differentiation.
  ...
  Its bounded actual-mapping query now also returns formal/actual names,
  positions, keyword status, optionality, and storage identity for direct
  same-file calls, with explicit unsupported mapping facts.
  ...
  facts distinguish missing assignment from explicit reassignment. Abstract
  ...
  The same component-dispatch query now supports one contiguous rank-one
  literal section such as `typed%leaf(2:4)` while refusing dynamic, strided,
  higher-rank, generic, alias, pointer, allocatable, global, and ownership
  paths; its focused API oracle passes.
- FFC `2f6cc37` (on top of `f08a9f0`), including rank-three and rank-four intrinsic
  ...
  host-scope interfaces, now lower through the callable-address ABI. A
  rank-one integer `class(*)` assumed-shape dummy now uses the canonical array
  descriptor and supports `SELECT TYPE` array-view narrowing, with a
  ...
  forms remain explicit boundaries. Rank-two integer and `real(8)` whole
  arrays now use the same descriptor shape/stride contract, with independent
  differential and refusal tests; sections, allocatables, and pointer/target
  actuals remain unsupported. Runtime automatic arrays with one through four
  dimensions now lower through the descriptor path, with an independent
  rank-four differential oracle and existing runtime-array regressions.
  Runtime-bounded scalar broadcasts into rank-one through rank-four sections
  with multiple retained dimensions now use the descriptor decoder and pass an
  ...
  independent gfortran differential oracle; rank-three assumed-shape now also
  passes an independent gfortran differential oracle, while rank-four and
  higher, `DIM`, and `KIND` forms remain precise refusals.
...
top of `cbd9910`, `80cffc6`, `22e9627`, `2e7446e`, `51bea55`, `f94e35c`, `caff12b`, `92bf9ad`, `bfe204d`, `1215d44`, `f51bb4c`, `a693014`, `4c8635a`, `08c616d`, `7c65a88`, `1ef4a45`, `c19beea`, `bdb4044`, `c1c3d00`, `35fa6e8`, `f82cae6`, `ea727c8`, `4a4fdd1`, `88f8b7d`, `0ff5e9f`, `e8678ef`, `443c9a8`,
  ...
  plus fixed-shape array-element type-bound receiver JVP/VJP. It also supports
  one scalar fixed-source polymorphic allocatable component ownership path in
  JVP and VJP, with paired concrete shadows and a generated-source,
  finite-difference, and adjoint-identity oracle. General arrays, sections,
  general reallocation, aliases, pointers, factories, unresolved dispatch, and global
  state remain explicit refusals. A fixed one-dimensional polymorphic
  allocatable owner array with a literal extent, literal selected element, and
  declared concrete `SOURCE=` now differentiates in JVP and VJP. The reverse
  path keeps the dynamic type behind `SELECT TYPE`, zeros the concrete owner
  payload, routes the selected owner cotangent, and replays it into the source
  object. Dynamic indices, ambiguous or factory sources, TARGET aliases,
  pointers, sections, changing lifetimes, and unresolved dispatch remain
  precise refusals. Scalar whole
  allocatable-component automatic reallocation now works in JVP and VJP for the
  bounded scalar concrete case.
  The current GNU-supported extension also replays a rank-one concrete `REAL`
  allocatable component across the same lifetime sequence for literal shapes
  in JVP and VJP, with dynamic-shape, polymorphic, alias, `TARGET`, and rank-two
  forms refused by the independent oracle.
  It now also replays a bounded rank-two concrete `REAL` allocatable component
  for literal shapes in JVP and VJP, with higher ranks, dynamic shapes,
  polymorphism, aliases, pointers, `TARGET`, globals, and whole-array access
  ...
  Scalar `INTENT(IN)` arithmetic actuals in direct same-file calls now lower
  through exact actual/formal storage facts in JVP and VJP, with hand,
  finite-difference, adjoint, and refusal oracles; unsafe mappings remain
  explicit refusals. The full GNU `fo check` gate is green: 130 modules and
  96/96 tests, with
  changed-file formatting passing; repository lint still reports 108 existing
  compiler warnings.
  ...
  now has 125 measured rows and zero runtime gaps; the former million-point
  LSTM and Brusselator signal-11 failures were inherited process-stack limits,
  and the runner now normalizes that limit. The complete matrix still does
  not establish a whole-suite performance win: FortAD beats Enzyme on several
  workloads while Tapenade remains faster on others.
  ...
  **P8.6b bounded optional passed-procedure callback:** the same fixed target
  ...
  The optional-callback extension also accepts a keyword-supplied fixed target
  and one direct `PRESENT(callback)` guard when the omitted path is provably
  unreachable; the independent oracle checks both numerical paths and the
  named refusal cases.
  ...
  `TYPE IS` or `CLASS IS` arm now differentiates in JVP and VJP through paired
  concrete shadows. Read-modify-write, aliases, pointers, arrays, unresolved
  dispatch, and ownership-changing assignments remain precise refusals.
- FortAD P8.3e now differentiates an active one-dimensional
  ...
  FortAD P8.3f now also supports the active receiver VJP for that same fixed
  path: cotangents scatter into the selected concrete receiver element and the
  independent finite-difference and adjoint-identity oracle passes. Pointers,
...
- FortAD `7c65a88` now also differentiates a bounded active rank-one
  ...
  non-contiguous sections, aliases, pointers, and descriptor/lifetime cases
  remain refusals. The vector-JVP benchmark on the same branch reports an
  independently rerun GNU Fortran median batch/scalar ratio of 0.234.
- It also differentiates a borrowed nested polymorphic component selected by
...
- FortAD `717daec` also consumes FortFront's fixed `SELECT TYPE` component
  dispatch facts for one inherited/concrete implementation with explicit PASS
...
- fortad-bench `76c599f` (on top of `7ecdba6`, `fc4e3dd`, `9c2a694`, and `e3db0e7`), including the
  ...
  and no explicit gaps across Euler, RK4, LSTM, BA, and Brusselator at all five
  sizes. No cross-engine performance victory is claimed: the complete matrix
  is evidence for the remaining optimization work, not a universal win.
  The `next33` shard adds `set11/mvo02`, `set07/v460`, `set04/v031`, and
  `set05/v148` with fresh Tapenade three-mode probes, exact hashes, and
  independent behavioral/refusal oracles. Their current boundaries are
  procedure-call actual mapping, invalid generated interface, and dependent
  inference; no repaired-source support is claimed.
  ...
  The `next37` shard adds `set05/v182`, `set07/v499`, `set10/lh233`, and
  `set11/vpf23`; all four have fresh Tapenade three-mode probes, exact hashes,
  and independent behavioral or refusal oracles. They remain explicit
  boundaries for generic calls without derivative rules, active formatted I/O,
  program-unit layout, and external allocatable-dummy calls.
  The `next38` shard adds `set10/lh215`, `set10/lh216`, `set12/mvo11`, and
  `set07/v472`; all four have fresh Tapenade three-mode probes, exact hashes,
  and independent behavioral or refusal oracles.
  The `next39` shard adds `set10/v297`, `set11/vpf06`, `set05/v173`, and
  ...
  The `next42` shard adds `set03/lh068`, `set04/v002`, `set04/v003`, and
...
FortFront now retains loop-sensitive refusal facts. The fixed-source scalar
polymorphic assignment and replay slice is also complete: FortFront supplies
an NVHPC-safe out-argument fact contract and FortAD differentiates one
concrete `SOURCE=`/`MOVE_ALLOC`/`SELECT TYPE` scalar path. Exact same-file
defined operators are now also consumed through FortFront's resolution
contract. The next step is general
...
assignment, nested receiver component-path facts for type-bound calls, and
bounded `ASSOCIATE` selector aliases, component paths, storage, dynamic-type,
and access facts, plus `SELECT RANK` arm rank-kind, selector-storage,
component-path, source-boundary, and dispatch-boundary facts. It exposes
resolved callback signature facts for direct internal targets, including
ordered dummy type/category/kind/rank/intent/OPTIONAL/VALUE metadata and
function-result metadata, as well as ownership-event paths, explicit
`MOVE_ALLOC` transfer facts, and automatic-reallocation candidates, including
owner paths, ranks, assignment and indexed type-bound receiver storage/type
facts, deep derived-assignment ownership facts, ownership-event sequencing
including source order, allocation states, `MOVE_ALLOC` effects, and implicit
reallocation, and explicit global-state and alias refusals,
...
explicit invalid or unresolved flags. Its independent arm-dispatch API
executable passes and reports the selected concrete or inherited binding,
implementation, `PASS` mode, and signature for one direct call inside a
concrete arm. Deferred, generic, ambiguous, dynamic, nested, array, and
ownership cases remain explicit boundaries.
Its public branch-predicate query now distinguishes exact `TYPE IS`, extension
`CLASS IS`, `CLASS DEFAULT`, abstract guards, and out-of-hierarchy or
unresolved refusals for downstream differentiation.
...
Its independent API contract passes, including `DO` and `DO WHILE` loop
boundaries parsed inside callback arms. General flow-sensitive callback state
remains open.
...
storage, procedure-call, and three-level abstract-hierarchy oracles pass. The
full parallel gate has a
...
open. The new `ASSOCIATE`, `SELECT RANK`, and `SELECT TYPE` API oracles are
committed, but
their focused `fo` runs are currently blocked by the existing vendored-stdlib
dependency scanner crash in `error_reporting`. No assertion result is claimed
for those scanner-blocked API runs until that scanner boundary is repaired. The
focused callback-signature and callback-target executables pass, and the
fallback full gate passes. External,
...
default-real call in GCC `proc_ptr_25.f90` (#448), and a same-unit typed
`real(8)` result, with compiled behavioral oracles. Other result kinds and
flow-sensitive targets remain explicit boundaries. The rank-one and rank-two
integer, real, and logical whole-allocatable
...
rank greater than four remain explicit refusals for components. FFC `fd3b5e8`
...
explicit boundaries. The genuine assumed-rank slice lowers rank-one through
rank-four REAL whole-array `SELECT RANK` arms through the canonical
...
the focused rank-two/rank-three component and rank-four owner oracles pass. Aggregate CI still
retains known formatter/full-suite and corpus failures. The latest full GNU
`fo` run reports 26 baseline compiler and
...
The current FortFront `main` handoff is `edcb532b`, which includes the
      ...
      proven multi-level inheritance depth for abstract/deferred dispatch,
      ...
      The current handoff also exposes `query_polymorphic_assignment_into`,
      an NVHPC-safe out-argument query for concrete-source assignment into a
...
- [ ] The current FortAD head passes GNU/Flang/ifx/nvfortran/LFortran.
      ...
      targets, and runs 96/96 tests. The NVIDIA compatibility build now
      compiles 410 FortAD targets and 409 derivative targets after the fo
      profile-selection fix, but its execution lane still has target-level
      NVHPC crashes and compiler-dialect incompatibilities. The remaining
      failures must be fixed or explicitly bounded; they must not be hidden
      by XFAILs.
...
`2a493998`), FFC's rank-two through rank-four runtime-descriptor/
allocatable-component-owner and rank-two assumed-rank descriptor slices (main
`f0ed424`), FortAD's rank-two
contiguous-section lowering and active concrete rank-two through rank-four
allocatable-component JVP/VJP (main `48dec22`), a bounded concrete scalar
...
- [ ] **P7.2 Allocation lifetime.** Cover allocatable components,
      ...
      - [x] **P7.2f active concrete allocatable components.** FortAD
        `9cbfb0e` consumes FortFront's resolved storage/component facts for
        already-allocated scalar, rank-one, or rank-two intrinsic REAL component
        ...
        global storage, unsupported kinds, and rank greater than two remain
        explicit refusals. General component lifetime replay remains open.
      - [x] **P7.2g rank-four intrinsic allocatable owners.** FFC `080f8e2`
        lowers local and dummy integer, real, and logical rank-four
        allocatables, including allocation, element access, `ALLOCATED`,
        `SIZE`/`DIM`, dummy mutation, whole-owner copy, and deallocation. Its
        independent gfortran compiler/runtime oracle passes. Derived,
        polymorphic, pointer/alias, unsupported-kind, and rank-five paths
        remain explicit boundaries.
      - [x] **P7.2h active rank-two allocatable components.** FortAD `9cbfb0e`
        extends the bounded active concrete `REAL` component slice from rank
        one to rank two. The independent oracle checks hand values, central
        differences, the adjoint identity, and precise whole-component,
        rank-three, and non-REAL refusals. Component lifetime replay remains
        open.
      - [x] **P7.2j active rank-three allocatable components.** FortAD
        `10c0e46` extends the same bounded concrete `REAL` element JVP/VJP
        contract to rank three. Its independent hand, central-difference, and
        adjoint oracle passes. Rank-five, whole-component, lifetime, alias,
        polymorphic, global, and non-REAL paths remain named boundaries.
      - [x] **P7.2i rank-three intrinsic allocatable components.** FFC
        `080f8e2` lowers integer, real, and logical rank-three components with
        allocation, element indexing, `ALLOCATED`, `SIZE`/`DIM`, mutation, and
        deallocation. Its independent gfortran differential oracle also
        verifies the full logical pattern and the rank-four, TARGET, alias,
        and unsupported-kind boundaries.
      - [x] **P7.2k active rank-four allocatable components.** FortAD `b1c7478`
        extends the bounded concrete `REAL` element JVP/VJP contract through
        rank four. The independent
        [`test_active_allocatable_component_rank4_oracle.f90`](test/test_active_allocatable_component_rank4_oracle.f90)
...
- [ ] **P7.4 Procedure interfaces.** Preserve optional and keyword arguments,
      ...
      - [x] **P7.4j resolved callback signature facts.** FortFront `7139a946`
      ...
      - [x] **P7.4k branch-merged callback flow facts.** FortFront `7139a946`
        ...
        and signature facts and declares loops, nested branches, reassignment,
        nullification, generic or ambiguous targets, incompatible signatures,
        and calls inside a branch as boundaries. Its independent fixture covers
        both counted and `DO WHILE` loop boundaries.
      - [x] **P7.4l branch-merged callback lowering.** FortAD `48dec22`
        consumes the P7.4k fact and lowers one same-scope `IF/ELSE` callback
        assignment plus a direct call into tagged direct calls. The compiled
        JVP/VJP oracle checks hand derivatives, central-difference convergence,
        and the adjoint identity for both callback targets. It also checks
        source-local refusals for reassignment, `NULL()`, `NULLIFY`, generic,
        incompatible, nested, in-arm, alias, and active-global cases. FortFront
        supplies counted-loop and `DO WHILE` refusal facts, while general
        flow-sensitive callback state remains open.
- [ ] **P7.5 Complex values.** Define the real-Jacobian contract for complex
      ...
      - [x] **P7.5b bounded real-objective projection VJP.** A real-valued
            ...
            identity. Complex arithmetic, `conjg`, complex outputs, and BLAS
            remain named refusal boundaries; zero-valued `abs(z)` remains a
      ...
      - [x] **P7.5c bounded `aimag` projection VJP.** Commit `d1dec10`
            ...
            [`test_complex_reverse_oracle.f90`](test/test_complex_reverse_oracle.f90)
            checks hand values, central differences, and the real adjoint
            identity, while `conjg` remains a precise refusal.
      - [x] **P7.5d bounded nonzero complex `abs` projection VJP.** Commits
            `ecf5455` and `ea21512` differentiate a real objective through
            `abs(z)` for active complex inputs with a nonzero primal modulus.
            The independent complex reverse oracle checks hand values, central
            differences, and the adjoint identity; `abs(0)`, `conjg`, complex
            arithmetic, complex outputs, and BLAS remain explicit boundaries.
...
- [ ] **P7.7 Language-completeness lane.** Add assumed-rank and `select rank`,
      ...
      - [x] **P7.7b upstream SELECT RANK arm metadata.** FortFront `57367e12`
            exposes explicit-rank, `RANK DEFAULT`, and `RANK (*)` arm kinds,
            ...
            FortAD consumption are covered by P7.7c through P7.7g.
      ...
      - [x] **P7.7f rank-three assumed-rank lowering.** FFC `fd3b5e8` extends
            ...
            independent gfortran oracle preserves rank-one and rank-two
            behavior. `RANK DEFAULT`, `RANK (*)`, scalar, dynamic, pointer,
            alias, and unsupported-rank paths remain explicit boundaries.
      - [x] **P7.7g rank-four assumed-rank lowering.** FFC `fd3b5e8` extends
            the same canonical descriptor slice to one `RANK (4)` arm with
            four runtime extents and column-major indexing. The independent
            gfortran oracle preserves ranks one through three and checks
...
- [ ] **P8.3 Concrete type-bound calls.** Resolve and transform `pass`, named
      ...
      - [x] **P8.3b active concrete receiver components.** FortAD `1080a62`
            ...
            refusals.
      - [x] **P8.3c fixed-shape array-element receivers.** FortAD `48dec22`
            differentiates one statically indexed element of a fixed-shape
            concrete derived-type array through same-file type-bound functions
            and subroutines. The independent oracle checks JVP and VJP receiver
            shadows against hand values, central differences, and the adjoint
      ...
      - [x] **P8.3f active polymorphic array receiver VJP scattering.** The
            ...
            slice.
      - [x] **P8.3g active scalar polymorphic receiver.** FortAD differentiates
            a scalar `class(base_t)` receiver through one fixed concrete
            deferred-binding path in JVP and VJP, with finite-difference and
            adjoint-identity checks. Multi-target dispatch, aliases, and
            pointers remain explicit refusals.
      - [x] **P8.3h bounded polymorphic array sections.** A borrowed,
            contiguous rank-one `class(base_t)` section with a literal lower
            bound is differentiated inside one fixed `SELECT TYPE` arm. JVP
            and VJP map literal section elements back to caller-owned storage.
            Non-contiguous sections, dynamic bounds, vector subscripts,
            pointers, aliases, and descriptor/lifetime changes remain named
            refusals. The independent oracle is
            `test_polymorphic_section_receiver_oracle.f90`.
      - [x] **P8.3i borrowed nested polymorphic components.** A concrete holder
            with a caller-owned scalar polymorphic component can be selected by
            one fixed `TYPE IS` arm and differentiated through paired holder
            shadows in JVP and VJP. Allocation, deallocation, `SOURCE=`,
      ...
      - [x] **P8.3k fixed-arm `CLASS IS`.** One concrete `CLASS IS` arm is
            differentiated under the same passive dynamic-type contract as
            `TYPE IS`, including active receiver components and paired reverse
            cotangents. Multiple arms, aliases, pointers, global mutable state,
            and ownership changes remain explicit refusals. The independent
            oracle is `test_polymorphic_class_is_oracle.f90`.
      - [x] **P8.3l fixed-source nested polymorphic component ownership.** A
            scalar `class(base_t), allocatable` component at a statically known
            nested path accepts one concrete local `SOURCE=` acquisition and
            final deallocation. JVP and VJP use paired concrete shadows and
            pass generated-source, finite-difference, and adjoint-identity
            checks. Arrays, sections, repeated acquisition, aliases, pointers,
            factories, unresolved dispatch, and global state remain refusals.
      - [x] **P8.3m fixed-source polymorphic component assignment.** Inside one
            proven `TYPE IS` or `CLASS IS` arm, a direct scalar assignment to
            the selected concrete component differentiates in JVP and VJP
            through paired concrete shadows. Read-modify-write, aliases,
      ...
      - [x] **P8.3o fixed-source unlimited-polymorphic assignment.** A scalar
            `class(*)` component selected by one proven `TYPE IS` or `CLASS IS`
            arm differentiates through paired concrete shadows in JVP and VJP.
            Unresolved multi-arm dispatch, ownership changes, aliases,
            pointers, factories, and global state remain explicit refusals.
            The independent oracle is
            `test_polymorphic_unlimited_assignment_oracle.f90`.
- [ ] **P8.4 Abstract deferred bindings.** Generate a derivative binding for
      ...
      - [x] **P8.4e concrete dispatch-arm contract.** FortFront `55c0c5b6`
            ...
            [`test_abstract_concrete_dispatch_oracle.f90`](test/test_abstract_concrete_dispatch_oracle.f90)
            differentiates a concrete leaf inheriting that binding through JVP,
            VJP, finite differences, and the adjoint identity. General
            derivative-hierarchy generation and unresolved runtime dispatch
            remain open.
- [ ] **P8.5 Polymorphic ownership.** Cover allocatable base-class components,
      ...
      - [x] **P8.5e abstract hierarchy implementation facts.** FortFront
            `15199205` exposes effective implementation nodes, declaring and
            inherited status, implicit or named `PASS` signatures, passed-object
      ...
      - [x] **P8.5f ASSOCIATE selector facts.** FortFront `7139a946` retains
...
- [ ] **P8.6 Procedure pointers and callbacks.** Treat callback identity as a
      ...
      branch-merged callback-flow, loop-sensitive callback, and `SELECT TYPE`
...
- [ ] **B0 Tapenade corpus classification.** The companion manifest
      ...
      parallel and reproducible. Its latest `next19` shard adds four exact
      ...
      labeled-DO boundaries. The current queue has 1,097 rows, including
      1,023 pure-Fortran and 74 mixed-language candidates. There are 409 of
      1,432 strict pure-Fortran candidates and 917 of 2,014 total candidates
      ...
      `set11/v540a`, `set11/v541a`, `set06/v339`, and `set05/v197`. `next4` closed
      `set06/v342`, `set12/mvo33`, `set11/vpf09`, and `set06/v335`. `next3`
      closed `set04/lh109`, `set12/mvo32`, `set12/mvo31`, and `set04/lh121`.
      `next2` closed `set11/vpf19`, `set07/v479`, `set04/lh111`, and
      `set07/v520`. Use independent derivative oracles
